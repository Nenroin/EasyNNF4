...
from src.base.layers import RNNLayer
from src.base.optimizers import Optimizer
from src.base.value_initializer import ValueInitializer, rnn_initializer


class ElmanLayer(RNNLayer):
    def __init__(
            self,
            neurons: int,
            activation: Activation = None,
            is_trainable: bool = True,
            prev_weights_initializer: ValueInitializer = rnn_initializer(),
            recurrent_weights_initializer: ValueInitializer = rnn_initializer(),

            stacked_layers: int = 1,

            use_bias: bool = True,
            bias_initializer: ValueInitializer = rnn_initializer(),
    ):
        super().__init__(
            neurons=neurons,
    ...
    def forward(self, in_batch: np.array, initial_state=None, training=True) -> np.array:
        # transpose in_batch from (batch_size, time_steps, features) to (time_steps, batch_size, features)
        in_batch = np.transpose(in_batch, (1, 0, 2))

        time_steps = in_batch.shape[0]
        ...
        output = []
        self.prev_in = [[[] for _ in range(time_steps)] for _ in range(self.stacked_layers)]
        self.prev_s = [[[] for _ in range(time_steps)] for _ in range(self.stacked_layers)]
        self.prev_out = [[[] for _ in range(time_steps)] for _ in range(self.stacked_layers)]
        self.prev_h_t_minus_1 = [[[] for _ in range(time_steps)] for _ in range(self.stacked_layers)]
    ...
    def backward(self, layer_gradient: np.array, optimizer: Optimizer) -> np.array:
        # transpose layer_gradient from (batch_size, time_steps, out_features) to (time_steps, batch_size, out_features)
        layer_gradient = np.transpose(layer_gradient, (1, 0, 2))

        time_steps = layer_gradient.shape[0]
        batch_size = layer_gradient.shape[1]

        de_dy = layer_gradient
        de_ds = [[] for _ in range(time_steps)]
